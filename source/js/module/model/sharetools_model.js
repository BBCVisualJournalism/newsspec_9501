...
* @requires lib/news_special/bootstrap
...
define(['lib/news_special/bootstrap'], function (news) {
    var STORY_PAGE_URL = 'http://www.bbc.co.uk/nhswinter/', //window.location.href || 'www.bbc.co.uk/news',
    ...
    */
    var NSShareModel = function (config) {     
        ...
        if (opts.header) {this.setHeader(opts.header); } else { this.setHeader('Share this pageOLD'); }
        if (opts.app) {this.setAppId(opts.app); } else {this.setAppId('PersonalSharing'); }
        if (opts.desc) {this.setOGPDescription(opts.desc); } else {this.setOGPDescription('Shared via BBC NewsOLD'); }
    ...
    */
    NSShareModel.prototype.getHashTags = function () {
        return (typeof this._hashTags !== 'undefined') ? this._hashTags.toString().replace(/,/g, '%20'):'';
    },
    /**
    ...
    * @throws ValueError - If the hashtag already exists in the collection {hashtags}
    ...
    NSShareModel.prototype.addHashTag = function (tag) {
        ...
        if (typeCheck(tag, 'string')) {
            if (Array.prototype.indexOf) { // IE8 breaks if we call indexOf
    ...
    */
    NSShareModel.prototype.emailShareTarget = function () {
        ...
        '?subject=' + this.getOGPDescription() + 
